...
/// Configuration for the `amazon-ebssurrogate` builder.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct AmazonEbsSurrogateConfig {
    /// The name of the builder instance.
    pub name: String,
    /// The instance type to use for the surrogate instance.
    ...
    /// The source AMI for the surrogate instance.
    ...
    /// Root device block mapping for the new AMI.
    pub ami_root_device: Option<BlockDeviceMapping>,
    /// Launch block device mappings.
    pub launch_block_device_mappings: Vec<BlockDeviceMapping>,
    /// Surrogate volume type (e.g. gp3).
    pub surrogate_volume_type: Option<EbsVolumeType>,
    /// Surrogate volume size in GB.
    pub surrogate_volume_size: Option<u64>,
    /// Device name for the surrogate volume attachment.
    pub surrogate_device_name: Option<String>,
...
/// The `amazon-ebssurrogate` builder.
#[derive(Debug, Clone)]
pub struct AmazonEbsSurrogateBuilder {
    /// The builder configuration.
    pub config: AmazonEbsSurrogateConfig,
}

impl AmazonEbsSurrogateBuilder {
    /// Create a new `AmazonEbsSurrogateBuilder`.
    #[must_use]
    pub const fn new(config: AmazonEbsSurrogateConfig) -> Self {
        Self { config }
    }
}

/// Step to launch the source surrogate EC2 instance and attach the surrogate volume.
#[derive(Debug, Clone)]
...
impl Step for StepProvision {
    ...
    async fn run(&mut self, state: &mut StateBag) -> Result<StepAction, StampError> {
        ...
        let build_ctx = BuildContext {
            ...
            source_type: "amazon-ebssurrogate".to_string(),
            ..Default::default()
        };
...
/// Step to stop the surrogate instance.
#[derive(Debug, Clone)]
...
/// Step to snapshot the surrogate volume and register the new AMI.
#[derive(Debug, Clone)]
struct StepCreateSnapshotAndRegisterAmi {
    /// UI logger.
    ui: Arc<crate::engine::ui::Ui>,
    /// Builder name.
    name: String,
...
#[async_trait::async_trait]
impl Step for StepCreateSnapshotAndRegisterAmi {
    #[cfg_attr(coverage_nightly, coverage(off))]
    async fn run(&mut self, state: &mut StateBag) -> Result<StepAction, StampError> {
        let vol_id = state
            .get::<String>("surrogate_volume_id")
            .cloned()
            .unwrap_or_default();
        let ami_name = self
            .config
        ...
        #[cfg(not(test))]
        {
            ...
            let snap_res = client
                .create_snapshot()
                .volume_id(&vol_id)
                .description(format!("Snapshot for {ami_name}"))
                .send()
                .await
                .map_err(|e| StampError::Execution(format!("Create snapshot failed: {e}")))?;

            let snap_id = snap_res.snapshot_id().unwrap_or_default();
            self.ui
            ...
            let root_device_name = self
                ...
                .ami_root_device
                .as_ref()
                .map_or("/dev/xvda", |d| d.device_name.as_str());

            let ebs_block = aws_sdk_ec2::types::EbsBlockDevice::builder()
                .snapshot_id(snap_id)
                .delete_on_termination(true)
                .volume_type(aws_sdk_ec2::types::VolumeType::Gp3)
                .build();

            let root_mapping = aws_sdk_ec2::types::BlockDeviceMapping::builder()
            ...
            let reg_res = client
                ...
                .block_device_mappings(root_mapping)
                .send()
                .await
...
#[async_trait::async_trait]
impl Builder for AmazonEbsSurrogateBuilder {
    #[cfg_attr(coverage_nightly, coverage(off))]
    ...
    async fn run(
        ...
        let mut runner = Runner::new(vec![
            ...
            Box::new(StepCreateSnapshotAndRegisterAmi {
                ui: ui.clone(),
                name: self.name(),
                config: self.config.clone(),
            }),
...
mod tests {
    ...
    fn test_derived_traits() {
        ...
        config.surrogate_volume_size = Some(16);
        config.surrogate_volume_type = Some(EbsVolumeType::Gp3);
        config.surrogate_device_name = Some("/dev/xvdf".to_string());
    ...
    #[tokio::test]
    async fn test_amazon_ebssurrogate_prepare_failure() {
        let config = AmazonEbsSurrogateConfig::default();
        let builder = AmazonEbsSurrogateBuilder::new(config);
    ...
    async fn test_amazon_ebssurrogate_run_mocked() {
        ...
        config.spot_instance = Some(SpotInstanceConfig {
            spot_price: Some("0.08".to_string()),
    ...
    async fn test_steps_run_and_cleanup() {
        ...
        state.put("surrogate_volume_id", "vol-mock".to_string());
        ...
        let mut step3 = StepCreateSnapshotAndRegisterAmi {
            ui: ui.clone(),
            name: "test".into(),
            config: config.clone(),
        };
