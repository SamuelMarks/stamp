...
impl Step for StepCreateSecurityGroup {
    ...
    async fn run(&mut self, state: &mut StateBag) -> Result<StepAction, StampError> {
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| StampError::Execution(format!("Failed to create security group: {e}")))?;
            ...
            for img in desc.images() {
                for bdm in img.block_device_mappings() {
                        ...
                        && let Some(snap_id) = ebs.snapshot_id()
                    {
                        self.ui.say(
                            &self.name,
                            &format!("Enabling Fast Snapshot Restore for snapshot {snap_id}..."),
                        );
...
mod tests {
    ...
    #[test]
    fn test_volume_types_and_defaults() -> Result<(), StampError> {
    ...
    #[tokio::test]
    async fn test_steps_execution_mock() -> Result<(), StampError> {
        ...
        assert_eq!(res_kp, StepAction::Continue);
        ...
        assert_eq!(res_share, StepAction::Continue);
        assert_eq!(
        ...
        assert_eq!(res_lt, StepAction::Continue);
        assert_eq!(
