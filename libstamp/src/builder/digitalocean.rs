...
pub fn get_do_token(config: &DigitalOceanConfig) -> Result<String, StampError> {
    ...
    if cfg!(test) {
        return Ok("mock-do-token".to_string());
    }
    ...
    async fn cleanup(&mut self, state: &StateBag) {
            ...
            .unwrap_or(false)
        {
            if let Some(key_id) = state.get::<u64>("ssh_key_id") {
                ...
                if !cfg!(test)
                    && let Ok(token) = get_do_token(&self.config)
                {
                    let client = reqwest::Client::new();
...
impl Step for StepCreateDroplet {
    ...
    async fn run(&mut self, state: &mut StateBag) -> Result<StepAction, StampError> {
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| StampError::Execution(format!("Create Droplet request failed: {e}")))?;
...
Do you want to clean up? [y/N]: ",
        ...
        let artifact_id = state
            ...
            .cloned()
            .unwrap_or_else(|| "digitalocean-mock-artifact".to_string());
...
mod tests {
    ...
    #[tokio::test]
    async fn test_digitaloceanbuilder_run() -> Result<(), StampError> {
    ...
    #[tokio::test]
    async fn test_digitaloceanbuilder_run_bad_exit() -> Result<(), StampError> {
        let config = DigitalOceanConfig {
