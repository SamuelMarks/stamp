//! Implementation of the `windows-shell` provisioner.
...
/// Configuration for the `windows-shell` provisioner.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WindowsShellConfig {
    ...
    /// The remote path where scripts will be uploaded. Defaults to `C:\Windows\Temp\script.bat`.
...
impl Default for WindowsShellConfig {
    fn default() -> Self {
        Self {
            inline: None,
            script: None,
...
/// The `windows-shell` provisioner.
#[derive(Debug, Clone)]
pub struct WindowsShellProvisioner {
    /// The provisioner configuration.
    pub config: WindowsShellConfig,
}

impl WindowsShellProvisioner {
    /// Create a new `WindowsShellProvisioner`.
    #[must_use]
    pub const fn new(config: WindowsShellConfig) -> Self {
        Self { config }
    }
    ...
    /// Formats environment variables for Windows CMD execution with escaping.
    #[must_use]
    ...
    pub fn build_command(&self, remote_path: &str) -> String {
        let default_cmd = "{{ .Vars }} cmd /c \"{{ .Path }}\"".to_string();
        ...
        self.wrap_elevated_command(&cmd)
    }
                ...
                match tokio::time::timeout(to.0, exec_fut).await {
                    ...
                    Err(_) => {
                        ...
                        return Err(StampError::CommunicatorTimeout {
                            target: "windows-shell".to_string(),
                ...
                Ok(exec_result) => {
                    if !exec_result.stdout.is_empty() {
                        for line in exec_result.stdout.lines() {
                            ui.say("windows-shell", line);
                        }
                    }
                    if !exec_result.stderr.is_empty() {
                        for line in exec_result.stderr.lines() {
                            ui.error("windows-shell", line);
                        }
                    }
...
impl Provisioner for WindowsShellProvisioner {
    async fn provision(
        &self,
        comm: &dyn Communicator,
        ...
        if let Some(inline) = &self.config.inline {
            ...
            for cmd in inline {
                ...
                let raw_cmd = if env_vars.is_empty() {
                    cmd.clone()
                } else {
                    format!("{env_vars} {cmd}")
                };

                let full_cmd = self.wrap_elevated_command(&raw_cmd);
        ...
        for (i, script) in all_scripts.iter().enumerate() {
            ...
            let remote_path_str = self
                ...
                .remote_path
                .clone()
                .unwrap_or_else(|| format!("C:\\Windows\\Temp\\script_{i}.bat"));
...
mod tests {
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_inline_success() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            inline: Some(vec!["echo hi".to_string()]),
            ..Default::default()
        };
        let prov = WindowsShellProvisioner::new(config);
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_inline_with_env() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            inline: Some(vec!["echo hi".to_string()]),
            environment_vars: Some(vec!["FOO=bar".to_string()]),
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_failure() -> Result<(), StampError> {
        let config = WindowsShellConfig {
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_no_commands() {
        let config = WindowsShellConfig::default();
        let prov = WindowsShellProvisioner::new(config);
        let comm = MockCommunicator::new();
        let result = prov
            .provision(
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_script() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            script: Some(FilePath::new(PathBuf::from("local.bat"))),
            clean_up: true,
            ..Default::default()
        };
        let prov = WindowsShellProvisioner::new(config);
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_scripts_and_env() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            scripts: Some(vec![FilePath::new(PathBuf::from("local.bat"))]),
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_script_failure() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            script: Some(FilePath::new(PathBuf::from("fail.bat"))),
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_valid_exit_codes_and_escaping() -> Result<(), StampError>
    {
        let config = WindowsShellConfig {
            inline: Some(vec!["fail_provision".to_string()]),
            valid_exit_codes: Some(vec![1]),
            environment_vars: Some(vec!["FOO=bar^&|<baz>".to_string(), "BARE".to_string()]),
            ..Default::default()
        };
        let prov = WindowsShellProvisioner::new(config);
        let comm = MockCommunicator::new();
    ...
    #[tokio::test]
    async fn test_windows_shell_provision_retries_and_timeout() -> Result<(), StampError> {
        let config = WindowsShellConfig {
            inline: Some(vec!["fail_provision".to_string()]),
            max_retries: 1,
    ...
    fn test_derived_traits() {
        let config1 = WindowsShellConfig {
            inline: Some(vec!["echo hi".to_string()]),
            ..Default::default()
        };
        ...
        let st1 = WindowsShellProvisioner::new(config1);
        let st2 = st1.clone();
